...
**Goal**: read (and where reasonable, write) Parquet natively, and read Iceberg and Delta Lake tables — validating that the catalog/format design from Phase 2 wasn't a toy imitation.
...
4. Iceberg read (done, `engine/crates/atlas-format/src/iceberg.rs`): implemented as a from-spec reader over `apache-avro` rather than a wrapper over the `iceberg-rust` crate — that crate's dependency graph (async runtime, REST/Glue/Hive catalog clients, object-store backends) is catalog machinery this read-only, filesystem-pointed path doesn't need, and `apache-avro` is the one piece of it (Avro manifest decoding) actually doing spec-defined work. `read_iceberg_table(metadata_path)` parses the table's `metadata.json`, follows its current snapshot's manifest list and manifests, and translates every live data file into Atlas's own manifest shape (file path, partition values, row/byte counts, per-column min/max/null-count stats) so the rest of the pipeline (pruning, scheduling, execution) doesn't need to know the source format. Iceberg's single-value bounds serialization is byte-identical to Atlas's own column-stats encoding for int/long/float/double/string, so bounds pass through unchanged. `atlas-cli ingest-iceberg --metadata <path> --dataset <name>` registers the table (no data copied); the worker dispatches manifests tagged `format = "iceberg"` to the same Parquet reader used for native Parquet manifests, since Iceberg data files are themselves Parquet.
5. Delta Lake read (done, `engine/crates/atlas-format/src/delta.rs`): same shape as Iceberg read, but simpler — Delta's transaction log (`_delta_log/*.json`) is plain JSON, not Avro, so this needs no new dependency (`serde_json` alone replays the add/remove/metaData actions), let alone a wrapper over the full `deltalake` crate. `read_delta_table(table_path)` replays every commit in order to recover the current schema and live-file set, translating each into Atlas's own manifest shape exactly like the Iceberg reader does. Unlike Iceberg (whose bounds arrive pre-serialized in Atlas's own byte convention), Delta's `stats.json` bounds are JSON-typed values, so those get an actual encode step (LE bytes for numerics, raw UTF-8 for strings, a Howard-Hinnant-algorithm epoch-day conversion for date strings). `atlas-cli ingest-delta --table-path <path> --dataset <name>` registers the table (no data copied); the worker dispatches manifests tagged `format = "delta"` to the same Parquet reader Iceberg/native-Parquet manifests already use. Checkpoint files (`_last_checkpoint`/`*.checkpoint.parquet`) are out of scope, same as Iceberg's own documented limitations — only the plain JSON commit log is replayed.
6. Tests (done): `atlas-format::format_tests::atlas_and_parquet_round_trips_agree_on_identical_source_data` writes one batch through both formats and asserts identical `RecordBatch` output. Iceberg: `engine/crates/atlas-format/tests/fixtures/iceberg_sample` is a real table (2 partitions, identity-partitioned on `hospital`, 5 rows) generated by PyIceberg — an independent writer, not Atlas's own — and `iceberg::tests` asserts correct schema translation, per-partition row counts (partition values feed the same `partition_values_json`-based pruning Phase 4 already built), and that column-stats bytes decode to the expected values. Delta: `engine/crates/atlas-format/tests/fixtures/delta_sample`, the same fixture shape generated by the Python `deltalake` package instead, asserted the same way by `delta::tests`.

### Definition of Done

- [x] `atlas-cli ingest --format parquet` and default `.atlas` format produce query-equivalent results.
- [x] A dataset can contain both `.atlas` and Parquet manifests simultaneously and query correctly across both.
- [x] Atlas can query an Iceberg table it did not create (fixture generated by PyIceberg), with partition values read from Iceberg's actual partition spec (not a reimplementation) and fed into the existing, format-agnostic partition-pruning code unchanged.
- [x] Atlas can query a Delta Lake table it did not create (fixture generated by the Python `deltalake` package), with partition values and column stats read from the table's actual transaction log and fed into the same format-agnostic pruning/dispatch code, unchanged, that Iceberg and native Parquet already use.
...
4. Literature retrieval (done, `ai-service/atlas_ai/retrieval/`): ingests a corpus (plain-text files via `ingest_cli.py`) into a vector store — `pgvector` on the existing Postgres instance (avoids standing up a second datastore) with embeddings via the configured provider's embedding endpoint, keeping BYO-model consistent with the completion path. `retrieve(query, k=5)` does the nearest-neighbor search the Report agent's `[literature:doc_id]`-tagged claims trace back to.
...
- [x] Full pipeline runs on a fixture healthcare dataset + fixture literature corpus, producing a report with both data-sourced and literature-sourced claims, each tagged with its source.
- [x] Every `[data]`-tagged claim traces to an actual query result from the Execution agent; every `[literature:doc_id]`-tagged claim traces to a retrieved document actually returned by step 4.
- [x] Each agent is independently testable (unit tests from step 9 don't require the full pipeline or live LLM calls to run).
...
- **OpenTelemetry**: add a trace span at the REST entry point (Phase 3) and propagate the trace context through gRPC metadata to workers (Phase 3) and the AI service (Phase 6), so one trace ID covers a full request. Add this when each service is first built, not retrofitted later — retrofitting tracing is where most of the pain is. Done: `ai-service/atlas_ai/telemetry.py`'s `span_from_grpc_context` reads the `traceparent` gRPC-metadata header the coordinator's otelgrpc-instrumented client already sends, closing the last hop (REST → catalog → worker → AI service, one trace id throughout).
- **Prometheus metrics**: worker task duration/count (Phase 3), cache hit rate (Phase 4), LLM call latency/token counts (Phase 6) — instrument each as its owning component is built. Done: `ai-service/atlas_ai/telemetry.py` exposes LLM call latency, count (by outcome), and prompt/completion token counts on its own port (9096), the same per-component-port convention the catalog/coordinator/workers use.
- **Auth**: add the `users`/`workspaces` tables and JWT middleware once the REST API exists (Phase 3), even if initially permissive (single default workspace), so later phases don't require retrofitting workspace-scoping into every catalog/query call. Done: `datasets.workspace_id` plus catalog-level filtering (`ListDatasets`/`GetDataset`/`GetCurrentSnapshot`) enforce it end to end, and `POST /auth/signup`/`POST /auth/login` (`coordinator/internal/accounts`, bcrypt) mint the same tokens `cmd/tokengen` does, so there's a real credential-based path to one alongside the dev CLI.
