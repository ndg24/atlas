...
ai-service/          # Python: LLM abstraction, NL→plan (Phase 6), AI analyst (Phase 7), research pipeline (Phase 8)
  atlas_ai/
    ...
    retrieval/       # retrieve() -- pgvector-backed literature retrieval; still a stub returning no documents
...
# every claim tagged [data] -- literature retrieval is still a stub (no
# corpus/pgvector wiring yet), so no [literature:...] claim appears yet
curl -X POST localhost:8080/research -H "Authorization: Bearer $ATLAS_TOKEN" -d '{"question": "which hospital sees the most patients?", "dataset": "patients", "corpus_id": ""}'

# --- everything below is not implemented yet (Phase 8, remaining) ---
